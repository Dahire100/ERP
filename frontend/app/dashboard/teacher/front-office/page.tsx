...
export default function FrontOfficePage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [visitorsRes, enquiriesRes] = await Promise.all([
                fetch('http://localhost:5000/api/teacher/front-office/visitors', { headers }),
                fetch('http://localhost:5000/api/teacher/front-office/enquiries', { headers })
            ])
    ...
    const handleAddVisitor = async () => {
        try {
            const token = localStorage.getItem('token')
            const res = await fetch('http://localhost:5000/api/teacher/front-office/visitors', {
                method: 'POST',
                headers: {
