...
export default function TeacherFeesPage() {
    ...
    const fetchFees = async () => {
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/teacher/fees?classId=${selectedClass}`, { headers })
    ...
    const handleRemind = async (studentName: string) => {
        ...
        try {
            const token = localStorage.getItem('token')
            const res = await fetch('http://localhost:5000/api/teacher/fees/remind', {
                method: 'POST',
