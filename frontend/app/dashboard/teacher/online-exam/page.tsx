...
export default function TeacherOnlineExam() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [quizRes, classRes] = await Promise.all([
        fetch(`${API_URL}/api/teacher/online-exams`, {
  ...
  const handleCreateQuiz = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_URL}/api/online-exams`, {
        method: 'POST',
        headers: {
  ...
  const handleEditQuiz = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_URL}/api/online-exams/${selectedQuiz._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteQuiz = async (quizId: string) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_URL}/api/online-exams/${quizId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      })
