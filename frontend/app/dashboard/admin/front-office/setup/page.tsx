...
export default function SetupFrontOfficePage() {
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/api/front-office-setup`, {
    ...
    const handleEdit = async (id: string, data: any) => {
        try {
            const token = localStorage.getItem('token')
            const response = await fetch(`${API_URL}/api/front-office-setup/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token')
            const response = await fetch(`${API_URL}/api/front-office-setup/${deleteConfirm.id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            })
            if (response.ok) {
