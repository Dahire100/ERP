...
export default function PhoneCallLogPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/phone-call-log`, {
  ...
  const handleEdit = async (id: string, data: any) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/api/phone-call-log/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/api/phone-call-log/${deleteConfirm.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
