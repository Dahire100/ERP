...
export default function EntranceExamPage() {
    ...
    const fetchExams = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/api/entrance-exam`, {
    ...
    const handleEdit = async (id: string, data: any) => {
        try {
            const token = localStorage.getItem('token')
            const response = await fetch(`${API_URL}/api/entrance-exam/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token')
            const response = await fetch(`${API_URL}/api/entrance-exam/${deleteConfirm.id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            })
            if (response.ok) {
