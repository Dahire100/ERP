...
export default function PostalReceiveDispatchPage() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/postal-exchange`, {
  ...
  const handleEdit = async (id: string, data: any) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/api/postal-exchange/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/api/postal-exchange/${deleteConfirm.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
