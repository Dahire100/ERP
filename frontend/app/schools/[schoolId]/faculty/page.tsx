...
import { User, Lock, Eye, EyeOff, HelpCircle, ArrowRight, ArrowLeft, BookOpen } from "lucide-react";
...
export default function FacultyLoginPage() {
    ...
    return (
        <div className="min-h-screen flex bg-gray-50">
            {/* Left Side - Visuals */}
            <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-600 via-indigo-700 to-indigo-900 items-center justify-center relative overflow-hidden p-12">
                <div className="relative z-10 text-center text-white space-y-8">
                    ...
                    <div>
                        ...
                        <h2 className="text-2xl font-light text-blue-100">Faculty & Admin Portal</h2>
                    </div>
                    <p className="max-w-md mx-auto text-blue-200 leading-relaxed text-lg">
                ...
                {/* Decorative elements */}
                <div className="absolute top-0 left-0 w-full h-full opacity-30 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-white via-transparent to-transparent"></div>
                <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
                <div className="absolute -top-24 -right-24 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"></div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white relative">
                ...
                <div className="w-full max-w-md space-y-8">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <div className="flex items-center justify-between pt-2">
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 transition-all transform hover:-translate-y-0.5 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                            >
                    ...
                    <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <Link href={`/schools/${schoolId}/help`} className="flex items-center gap-2 text-gray-500 hover:text-blue-600 text-sm font-medium transition-colors">
                            <HelpCircle size={16} /> Need Help?
                        </Link>

                        <div className="flex flex-col items-end gap-3 text-sm">
                            <Link href={`/schools/${schoolId}/student`} className="flex items-center gap-2 text-blue-600 hover:text-blue-700 font-bold group transition-colors">
                                Student Login <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                            </Link>
                            <Link href={`/schools/${schoolId}/forgot-password?role=faculty`} className="text-gray-400 hover:text-gray-600 transition-colors">
                                Forgot Password?
                            </Link>
                        </div>
                    </div>
